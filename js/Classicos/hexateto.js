...
function salvarProgresso() {
  ...
  localStorage.setItem("termo_hexateto", JSON.stringify(save));
}

export function iniciarModoHexateto(bancoDePalavras, listaSolucoes) {
  ...
  let save = null;
  try {
    save = JSON.parse(localStorage.getItem("termo_hexateto"));
  } catch {
...
export function receberTeclaHexateto(tecla) {
  ...
  if (todosVencidos || estado.statusBoards.every((s) => s === "venceu")) {
    ...
    if (estado.linhaAtual >= estado.maxTentativas) {
      ...
      setTimeout(
        () =>
          mostrarMensagem(
            `Fim! As palavras eram: ${estado.palavrasAlvo.join(", ")}`,
          ),
        3500,
      );
    } else {
